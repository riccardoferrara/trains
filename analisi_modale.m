...
% SstepsPRELOAD = 1;
partenza = 0;
ss2 = 2^(nextpow2(ss-SstepsPRELOAD-partenza)-1);
% ss2 = 2^(nextpow2(ss)-1);
...
OUTPUT = Rstk(10,SstepsPRELOAD+partenza+1:partenza+SstepsPRELOAD+ss2);
% OUTPUT = Uvk(9,:);
...
% passa_basso = fir1(10,2000/Frequenza);
% 
% OUTPUT = filter(passa_basso, 1, OUTPUT);

NFFT = 2^nextpow2(digitalizzazione); % Next power of 2 from length of y
